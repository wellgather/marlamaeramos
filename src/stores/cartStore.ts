...
interface CartStore {
  ...
  applyDiscount: (code: string) => Promise<{ ok: boolean; applicable: boolean }>;
  clearCart: () => void;
  ...
  persist(
    (set, get) => ({
      ...
      applyDiscount: async (code) => {
        const { cartId } = get();
        if (!cartId) return { ok: false, applicable: false };
        set({ isLoading: true });
        try {
          const res = await applyDiscountCode(cartId, code);
          if (res.success) {
            set({
              discountCode: code || null,
              discountApplicable: res.applicable,
              checkoutUrl: res.checkoutUrl ?? get().checkoutUrl,
            });
            return { ok: true, applicable: res.applicable };
          }
      ...
      clearCart: () => set({ items: [], cartId: null, checkoutUrl: null, discountCode: null, discountApplicable: false }),
    ...
    }),
    {
      ...
      partialize: (state) => ({
        ...
        discountApplicable: state.discountApplicable,
      }),
    }
  )
);
