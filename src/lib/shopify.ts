...
export const SHOPIFY_STORE_PERMANENT_DOMAIN = 'radiant-wellness-hub-yt382.myshopify.com';
export const SHOPIFY_STOREFRONT_URL = `https://${SHOPIFY_STORE_PERMANENT_DOMAIN}/api/${SHOPIFY_API_VERSION}/graphql.json`;
export const SHOPIFY_STOREFRONT_TOKEN = 'd2da7a0dd2df8afb9a1c5e0f9e621f11';
...
const CART_CREATE_MUTATION = `
  mutation cartCreate($input: CartInput!, $discountCodes: [String!]) {
...
export async function applyDiscountCode(cartId: string, code: string): Promise<{ success: boolean; applicable: boolean; checkoutUrl?: string }> {
  const data = await storefrontApiRequest(CART_DISCOUNT_UPDATE_MUTATION, {
    cartId, discountCodes: code ? [code] : [],
  });
  const userErrors = data?.data?.cartDiscountCodesUpdate?.userErrors || [];
  if (userErrors.length > 0) return { success: false, applicable: false };
  const cart = data?.data?.cartDiscountCodesUpdate?.cart;
  const applied = cart?.discountCodes?.find((d: any) => d.code.toLowerCase() === code.toLowerCase());
  return {
    success: true,
    applicable: applied ? applied.applicable : !code,
